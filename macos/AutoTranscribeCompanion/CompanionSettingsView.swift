...
struct CompanionSettingsView: View {
    ...
    private var setupView: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                sectionHeader(
                    "Setup",
                    subtitle: "The Mac companion watches the Dropbox drop folder, starts the watcher, and routes finished transcripts."
                )
    ...
    private var folderSetup: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("This must be the local Dropbox-synced folder that the iPhone uploads into. It should contain `scripts`, `config`, and the root drop area.")
                .font(.caption)
                .foregroundStyle(.secondary)

            TextField("Project folder", text: $viewModel.basePathString)
