...
struct ContentView: View {
    ...
    private var memoAccentBright: Color {
        Color(red: 0.46, green: 0.78, blue: 1.0)
    }

    private var memoAccentDeep: Color {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            let recorderY = (geometry.size.height * 0.345) + contentVerticalAdjustment
            let bottomPanelY = (geometry.size.height * 0.69) + bottomSafeAreaAdjustment
        ...
        .alert(item: $viewModel.receiverCheckPrompt) { prompt in
            Alert(
                title: Text(prompt.title),
                message: Text(prompt.message),
                primaryButton: .default(Text("Check Receiver")) {
                    Task { await viewModel.checkSelectedReceiver() }
                },
                secondaryButton: .cancel(Text("Cancel")) {
                    viewModel.dismissReceiverCheckPrompt()
                }
            )
        }
        .alert(item: $viewModel.receiverCheckSuccessPrompt) { prompt in
            Alert(
                title: Text(prompt.title),
                message: Text(prompt.message),
                dismissButton: .default(Text("OK")) {
                    viewModel.dismissReceiverCheckSuccessPrompt()
                }
            )
        }
        .task {
    ...
    private var memoBackground: some View {
        ZStack {
            ...
            RadialGradient(
                colors: [
                    memoAccent.opacity(0.28),
                    memoAccentDeep.opacity(0.12),
                    .clear
                ],
                center: .top,
                startRadius: 16,
    ...
    private var recorderBody: some View {
        VStack(spacing: 12) {
            ...
            Text(statusLine)
                ...
                .frame(maxWidth: 280, minHeight: 34, alignment: .top)
                .padding(.horizontal, 18)
        }
        .accessibilityIdentifier("recorderBody")
    }

    private var memoMicBadge: some View {
        Image("MemoMic")
            .resizable()
            .interpolation(.none)
            .scaledToFit()
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 20, style: .continuous)
                    .stroke(memoAccent.opacity(0.42), lineWidth: 1)
            )
            .shadow(color: memoAccent.opacity(0.34), radius: 18, y: 8)
    ...
    private func modeMenuFill(for screen: AppScreen) -> Color {
        ...
        case .call:
            return selectedScreen == .call ? memoAccent : Color.black.opacity(0.68)
        }
    }
    ...
    private var conversationRecordingStatus: some View {
        if viewModel.state == .recording || viewModel.state == .paused || viewModel.state == .uploading {
            HStack(spacing: 10) {
                Circle()
                    .fill(viewModel.state == .uploading ? memoAccentBright : memoAccent)
    ...
    private var chatRecordButtonBackground: Color {
        switch viewModel.state {
        case .recording: return memoAccent
        case .paused: return .yellow.opacity(0.88)
        case .uploading: return memoAccentBright.opacity(0.72)
        case .idle: return memoAccent.opacity(0.82)
        }
    }
    ...
    private var callTurnComposer: some View {
        HStack(spacing: 12) {
            ...
            Button {
                ...
                .padding(.horizontal, 16)
                .frame(height: 44)
                .background(canSendCallTurn ? memoAccentBright : Color.white.opacity(0.08), in: Capsule())
            }
            .buttonStyle(.plain)
    ...
    private var callVisualColor: Color {
        ...
        case .paused:
            return .yellow
        case .idle:
            return viewModel.callSessionActive || viewModel.voiceCallModeEnabled ? memoAccent : .white
        }
    }
    ...
    private var callPhaseSubtitle: String {
        ...
        switch viewModel.state {
        case .recording:
            return "Pause briefly or tap the blue button to send."
        case .uploading:
    ...
    private var callOrbColor: Color {
        ...
        case .paused:
            return .yellow
        case .idle:
            return viewModel.callSessionActive ? memoAccent : .white
        }
    }
    ...
    private var recordButton: some View {
        Button {
            ...
            Circle()
                .fill(viewModel.state == .recording ? memoAccent : .white.opacity(0.1))
                .frame(width: 68, height: 68)
                .overlay(
                    Circle()
                        .stroke(viewModel.state == .recording ? memoAccentBright.opacity(0.55) : .white.opacity(0.2), lineWidth: 3)
                )
                .overlay(
                    Group {
                        if viewModel.state == .recording {
                ...
                .shadow(color: viewModel.state == .recording ? memoAccent.opacity(0.42) : .clear, radius: 18, y: 8)
        }
    ...
    private var voiceCallToggle: some View {
        Button {
            ...
            .background(
                Capsule()
                    .fill(viewModel.voiceCallModeEnabled || viewModel.callSessionActive ? memoAccentBright : Color.white.opacity(0.08))
            )
        }
        .buttonStyle(.plain)
    ...
    private var statusColor: Color {
        ...
        case .recording: return memoAccentBright
        case .paused: return .yellow
        case .uploading: return memoAccentBright
        }
    }
    ...
    private func waveColor(for index: Int) -> Color {
        if viewModel.state == .recording {
            return memoAccentBright.opacity(0.86)
        } else if viewModel.state == .uploading {
            return memoAccent.opacity(0.78)
        }
        return Color(red: 0.60, green: 0.78, blue: 1.0).opacity(0.28)
    }
