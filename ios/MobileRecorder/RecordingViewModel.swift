...
final class RecordingViewModel: NSObject, ObservableObject {
    ...
    override init() {
        self.serverURLString = UserDefaults.standard.string(forKey: Self.serverURLDefaultsKey) ?? "http://192.168.15.14:8943"
    ...
    private static let serverURLDefaultsKey = "mobileRecorder.serverURL"
    private static let directTextServerURLDefaultsKey = "mobileRecorder.directTextServerURL"
    private static let directTextAPITokenService = "com.garnetuniverse.MobileRecorder.directTextAPIToken"
    ...
    private static let submitAfterPasteDefaultsKey = "mobileRecorder.submitAfterPaste"
    private static let textResponseModeDefaultsKey = "mobileRecorder.textResponseMode"
    private static let routeTargetDefaultsKey = "mobileRecorder.routeTarget"
    private static let showRouteSelectorOnRecorderDefaultsKey = "mobileRecorder.showRouteSelectorOnRecorder"
    private static let showRouteButtonsOnRecorderDefaultsKey = "mobileRecorder.showRouteButtonsOnRecorder"
    private static let quickRouteTargetsDefaultsKey = "mobileRecorder.quickRouteTargets"
    private static let defaultQuickRouteTargets: [RouteTarget] = [.kitty, .codex, .plexi]
    private static let voiceCallModeDefaultsKey = "mobileRecorder.voiceCallMode"
    private static let normalTranscriptionModeDefaultsKey = "mobileRecorder.normalTranscriptionMode"
    private static let liveTranscriptPreviewDefaultsKey = "mobileRecorder.liveTranscriptPreview"
    private static let speakCallResponsesDefaultsKey = "mobileRecorder.speakCallResponses"
    ...
    func activateRouteTarget(_ route: RouteTarget) async {
        ...
        do {
            if !serverURLString.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
    ...
    private func uploadRecording(
        ...
        if dropboxManager.isReadyForUpload {
    ...
    private func uploadServerConfiguration() throws -> (baseURL: URL, apiToken: String?) {
        let legacyURL = serverURLString.trimmingCharacters(in: .whitespacesAndNewlines)
        guard let baseURL = URL(string: legacyURL) else {
            throw RecorderError.invalidServerURL
        }
        return (baseURL, nil)
    }
    ...
    private func directTextURL(pathComponents: [String]) throws -> URL {
        ...
        guard var url = URL(string: trimmedURL),
